...
set(mTextBox,'String','Press SPACE to continue')
set(mTextBox,'Position',[170 170 200 20])
...
set(thrusts(i), 'Matrix', move * rotate );
end
% Update the drawing.
xmin = data.x(1,t)-20;
xmax = data.x(1,t)+20;
...
%h(1) = prism(-sqrt(5)/2, -sqrt(5)/2, -0.25, 10, 0.5, 0.5);
%h(2) = prism(-sqrt(5)/2, sqrt(5)/2, -0.25, 0.5, 10, 0.5);
% Draw bulbs representing propellers at the end of each arm.
[x y z] = sphere;
x = 0.5 * x;
y = 0.5 * y;
z = 0.5 * z;
h(3) = surf(x - 2*sqrt(2.5), y- 2*sqrt(2.5), z, 'EdgeColor', 'none', 'FaceColor', 'k');
h(4) = surf(x + 2*sqrt(2.5), y+2*sqrt(2.5), z, 'EdgeColor', 'none', 'FaceColor', 'k');
h(5) = surf(x-2*sqrt(2.5), y + 2*sqrt(2.5), z, 'EdgeColor', 'none', 'FaceColor', 'k');
h(6) = surf(x+2*sqrt(2.5), y - 2*sqrt(2.5), z, 'EdgeColor', 'none', 'FaceColor', 'k');
...
thrusts(1) = surf(x - 2*sqrt(2.5), y- 2*sqrt(2.5), z, 'EdgeColor', 'none', 'FaceColor', 'm');
thrusts(2) = surf(x + 2*sqrt(2.5), y+2*sqrt(2.5), z,'EdgeColor', 'none', 'FaceColor', 'y');
thrusts(3) = surf(x-2*sqrt(2.5), y + 2*sqrt(2.5), z,'EdgeColor', 'none', 'FaceColor', 'm');
thrusts(4) = surf(x+2*sqrt(2.5), y - 2*sqrt(2.5), z,  'EdgeColor', 'none', 'FaceColor', 'y');
...
h(i) = fill3(X(faces(i, :)), Y(faces(i, :)), Z(faces(i, :)), 'c'); hold on;
end
